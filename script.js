...
let loginRootId = null;
const GLOBAL_PASSWORD = 'abcd5678';
...
function handleLoginSubmit(event) {
    ...
    const allowedIds = rawData?.data_dump || {};

    if (!allowedIds[userId] || password !== GLOBAL_PASSWORD) {
        if (errorBox) errorBox.textContent = 'Invalid ID or password.';
        return;
    }
...
function buildTree() {
    ...
    if (!currentRootId || !nodesMap[currentRootId]) {
        currentRootId = primaryRootId;
    }

    treeData = nodesMap[currentRootId] || nodesMap[primaryRootId];
...
async function exportToImage() {
    ...
    // Inject styles right after the closing '>' of the opening <svg ...> tag
    svgString = svgString.replace(/(<svg[^>]*>)/, `$1<style>${styleString}</style>`);
...
function resetTreeState() {
    ...
    currentRootId = loginRootId || (primaryRootConn ? primaryRootConn.userId : null);
    renderTree();
}
